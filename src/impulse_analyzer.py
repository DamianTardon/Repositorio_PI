...
class LightningImpulseAnalyzer:
    ...
    def _remove_offset(self) -> None:
        ...
        Como parte de la máquina de estados, actualiza los atributos: ``self.idx_peak``, 
        ``self.time_star_impulse``, ``self.offset_value`` y ``self.zeroed_curve``.

        Raises:
            ValueError: Si no existen suficientes muestras en el pre-trigger (menos del 30% relativo 
                al índice del pico). Esto ocurre por una configuración inadecuada del osciloscopio 
                (nivel de trigger o delay incorrectos).
        """
    ...
    def _polarity_normalization(self) -> None:
        ...
        Como parte de la máquina de estados, actualiza: ``self.Ue``, ``self.polarity``, 
        ``self.factor``, ``self.zeroed_curve_abs`` y ``self.peak_value``.
    ...
    def _normalize_waveform(self) -> None:
        ...
        Como parte de la máquina de estados, actualiza el atributo: ``self.norm_voltage``.
    ...
    def _cutting_signal(self) -> None:
        ...
        Como parte de la máquina de estados, actualiza: ``self.start_slice``, ``self.end_slice``, 
        ``self.fit_voltage`` y ``self.fit_time``.

        Raises:
            RuntimeError: Si la señal corresponde a un impulso cortado ('chopped').
            ValueError: Si la onda no está normalizada, o si no se encuentran los umbrales 
                requeridos debido a recortes prematuros de la captura.
    ...
    def _double_exponential_func(t: np.ndarray, U: float, tau1: float, tau2: float, td: float) -> np.ndarray:
        ...
        .. note::
            ...
            rellenan con ceros (``0.0``) para prevenir que la función exponencial diverja y desborde 
            numéricamente (Overflow).
    ...
    def _fit_base_curve(self) -> None:
        ...
        Como parte del pipeline de la máquina de estados, actualiza directamente los atributos 
        internos: ``self.fitted_params`` (diccionario con U, tau1, tau2, td) y ``self.fitted_curve``.
        ...
        Raises:
            ...
            ValueError: Si los datos no han sido segmentados previamente, o si el algoritmo 
                de optimización diverge debido a una forma de onda severamente distorsionada.
        """
    ...
    def _construct_base_curve(self) -> None:
        ...
        Como parte de la máquina de estados, actualiza los atributos: ``self.base_curve`` y ``self.Ub``.
    ...
    def _calculate_residual_curve(self) -> None:
        ...
        Como parte de la máquina de estados, actualiza el atributo: ``self.residual_curve``.
    ...
    def _filter_to_residual(self) -> None:
        ...
        Como parte de la máquina de estados, actualiza el atributo: ``self.filtered_residual``.

        Raises:
            ValueError: Si no existe la curva residual original en el estado.
        """
    ...
    def _construct_test_voltage_curve(self) -> None:
        ...
        Como parte de la máquina de estados, actualiza: ``self.test_voltage_curve_abs``, ``self.Ut``, 
        ``self.test_voltage_curve``, ``self.test_voltage_curve_norm`` y el diccionario ``self.results["Ut"]``.
    ...
    def _calculate_parameters(self) -> None:
        ...
        Como parte de la máquina de estados, actualiza: ``self.idx_peak_Ut`` y asigna 
        los valores validados de ``OS``, ``T1`` y ``T2`` al diccionario ``self.results``.
        ...
        try:
            ...
            raise ValueError("No se pudo calcular el Tiempo de Frente (T1). El frente de onda puede tener demasiado ruido.")
    ...
    def _find_time_lag(self, ref_analyzer: LightningImpulseAnalyzer) -> None:
        ...
        Como parte de la máquina de estados, actualiza el atributo: ``self.t_L``.
    ...
    def _adjust_time_lag(self) -> None:
        ...
        Como parte de la máquina de estados, actualiza el atributo: ``self.aligned_time_axis``.
...
# que requieren comparación con un impulso completo de referencia.
    ...
    def _find_deviation_point(self, ref_analyzer: LightningImpulseAnalyzer, threshold: float = 0.02) -> None:
        ...
        Si no supera el umbral, la clasifica como plena (``impulse_type = "full"``).
        Como parte de la máquina de estados, actualiza: ``self.impulse_type`` y ``self.idx_deviation``.
    ...
    def _select_data_up_to_deviation(self) -> None:
        ...
        Como parte de la máquina de estados, actualiza: ``self.chopped_front_voltage`` y ``self.chopped_front_time``.
    ...
    def _find_amplitude_ratio(self, ref_analyzer: LightningImpulseAnalyzer) -> None:
        ...
        Como parte de la máquina de estados, actualiza el atributo: ``self.E``.
    ...
    def _find_chopping_instant(self) -> None:
        ...
        Como parte de la máquina de estados, actualiza: ``self.Tcutting_moment`` y ``self.U_collapse``.
...
# Métodos de análisis completo para cada tipo de onda.
    def ref_lightning_impulse(self) -> None:
        ...
        Orquesta internamente los métodos base y define ``self.impulse_type = 'full'``.
        """
