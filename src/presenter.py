...
class MainPresenter(QObject):
    ...
    Attributes:
        ...
        voltage_values (Dict[str, list[str]]): Valores permitidos de la escala de amplitud (V/div) según la magnitud seleccionada.
        time_values (Dict[str, list[str]]): Valores permitidos de la escala de tiempo (s/div) según la magnitud.
    ...
    def _connect_signals(self) -> None:
        ...
        # Habilitadores. (Uso de lambda para descartar el argumento int de stateChanged).
        self.ui.attenuations_enabler.stateChanged.connect(lambda: self._toggle_attenuations())
        ...
        self.ui.time_value.currentTextChanged.connect(lambda: self._update_t_scale())
    ...
    def _change_voltage_unit(self, channel: int, unit: str) -> None:
        r"""Modifica dinámicamente el Combobox de magnitudes de tensión basándose en la unidad activa.
        ...
        try:
            ...
            # Mantiene el valor actual si es válido, sino fuerza el primer elemento de la nueva lista.
    ...
    def export_results(self) -> None:
        ...
        Si la persistencia en disco se efectúa exitosamente, llama a rutinas del SO para invocar 
        ...
        # Sanitización estricta de cadenas extraídas de la GUI.
        item_num = re.sub(r'[\\/*?:"<>|]', "", self.ui.item_number_value.text().strip())
        item_year = re.sub(r'[\\/*?:"<>|]', "", self.ui.item_year_value.text().strip())
        client = re.sub(r'[\\/*?:"<>|]', "", self.ui.client_value.text().strip())

        # Validar que los campos esenciales no hayan sido borrados accidentalmente después de crear el ensayo.
        if not item_num or not item_year:
            QMessageBox.warning(
                None,
                "Datos faltantes",
                "Los campos de número y año del 'Item' no pueden estar vacíos al exportar.")
            return

        item_full = f"{item_num}-{item_year}"
        h5_file_path = self.fm.analysis / f"{item_full}.h5"
        ...
        # Garantizar que el archivo resultante contenga la extensión obligatoria
        # Previene "Fallos Silenciosos" si el SO o el usuario no agregan la terminación.
